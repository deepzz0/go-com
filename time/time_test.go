...
func TestFormatTime(t *testing.T) {
	now := time.Now()
	str := DefaultFormatTime(now)
	t.Log(str)
	str1 := FormatTime(now, Layout_year)
	str2 := FormatTime(now, Layout_month)
	str3 := FormatTime(now, Layout_day)
	str4 := FormatTime(now, Layout_second)
	t.Logf("%s\n%s\n%s\n%s\n", str1, str2, str3, str4)
	t.Logf("%s", FormatTime(now, time.ANSIC))
}

func TestTime2Seconds(t *testing.T) {
	sec := GetSeconds(1, 2, 30, 12)

	t.Logf("GetSeconds:%d", sec)
}
